  ...
  useEffect(() => {
    async function load() {
      ...
      for (const s of weekSessions) {
        const tags = (s as unknown as { session_tags: { tags: { name: string; color: string | null } | null }[] }).session_tags ?? [];
          ...
          {/* <Card>
            <CardContent className="space-y-4 py-6">
              <div className="flex items-baseline justify-between">
                ...
                <span className="text-sm text-muted-foreground">{weekData.reduce((s, d) => s + d.minutes, 0)} min</span>
              </div>
